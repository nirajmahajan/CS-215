...
for iter = [1:60000]
    ...
    data = reshape(im2double(digits_train(:, :, iter)), 1, 784);
    if (label == 0)
        label = 10;
    end
...
for iter = [0:9]
    ...
    subplot(1,3,1), imshow(reshape((temp_mean-additional_term), 28,28));
    title('\mu - \surd{\lambda}v');
    subplot(1,3,2), imshow(reshape(temp_mean, 28,28));
    xlabel(sprintf('Analysis for the digit %d', rem(index, 10)));
    title('\mu');
    subplot(1,3,3), imshow(reshape((temp_mean+additional_term), 28,28));
    title('\mu + \surd{\lambda}v');
